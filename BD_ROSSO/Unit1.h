//---------------------------------------------------------------------------

#ifndef Unit1H
#define Unit1H
//---------------------------------------------------------------------------
#include <System.Classes.hpp>
#include <Vcl.Controls.hpp>
#include <Vcl.StdCtrls.hpp>
#include <Vcl.Forms.hpp>
#include <Vcl.ExtCtrls.hpp>
#include <Vcl.Imaging.jpeg.hpp>
#include <Vcl.Imaging.pngimage.hpp>
#include <Vcl.Buttons.hpp>
#include <Vcl.ComCtrls.hpp>
#include <Data.DB.hpp>
#include <Vcl.DBGrids.hpp>
#include <Vcl.Grids.hpp>
#include <Vcl.WinXCtrls.hpp>
#include <Vcl.CheckLst.hpp>
#include <Vcl.Menus.hpp>
#include <Vcl.Samples.Calendar.hpp>
#include <Vcl.WinXCalendars.hpp>
//---------------------------------------------------------------------------
class TForm1 : public TForm
{
__published:	// IDE-managed Components
	TPageControl *PageControl1;
	TTabSheet *TabSheet1;
	TTabSheet *depozit;
	TTabSheet *magazin;
	TSplitView *SplitView2;
	TLabel *Label1;
	TEdit *Edit1;
	TEdit *Edit2;
	TLabel *Label2;
	TLabel *Label3;
	TSpeedButton *SpeedButton1;
	TLabel *Label4;
	TImage *Image5;
	TPanel *Panel2;
	TImage *Image1;
	TSpeedButton *SpeedButton2;
	TPanel *Panel3;
	TSplitView *SplitView1;
	TImage *Image2;
	TImage *Image6;
	TSpeedButton *SBafiseaza;
	TSpeedButton *SBmodifica;
	TLabel *Label5;
	TLabel *Label6;
	TSpeedButton *SpeedButton8;
	TImage *Image8;
	TLabel *Label9;
	TSpeedButton *SBadauga;
	TPanel *Panel4;
	TPageControl *PageControl2;
	TTabSheet *Pantofi;
	TTabSheet *Cizme;
	TDBGrid *DBGrid1;
	TDBGrid *DBGrid2;
	TSplitView *SplitView4;
	TImage *Image10;
	TSpeedButton *SpeedButton12;
	TImage *Image11;
	TSpeedButton *SpeedButton13;
	TSplitView *SplitView5;
	TImage *Image12;
	TImage *Image13;
	TSpeedButton *SpeedButton15;
	TSpeedButton *SpeedButton14;
	TTabSheet *MPantofi;
	TTabSheet *MCizme;
	TDBGrid *DBGrid3;
	TDBGrid *DBGrid4;
	TGroupBox *GroupBox1;
	TLabel *Label11;
	TLabel *Label12;
	TLabel *Label13;
	TLabel *Label14;
	TLabel *Label15;
	TLabel *Label16;
	TLabel *Label17;
	TLabel *Label18;
	TLabel *Label19;
	TLabel *Label20;
	TLabel *Label21;
	TLabel *Label22;
	TEdit *Edit4;
	TEdit *Edit5;
	TEdit *Edit6;
	TEdit *Edit7;
	TEdit *Edit8;
	TEdit *Edit9;
	TEdit *Edit10;
	TEdit *Edit11;
	TEdit *Edit12;
	TEdit *Edit13;
	TEdit *Edit14;
	TImage *Image14;
	TImage *Image15;
	TSpeedButton *SpeedButton16;
	TSpeedButton *SpeedButton17;
	TEdit *Edit3;
	TTabSheet *MTip;
	TDBGrid *DBGrid5;
	TTabSheet *MMAterial;
	TTabSheet *MCuloare;
	TDBGrid *DBGrid6;
	TDBGrid *DBGrid7;
	TImage *Image16;
	TSpeedButton *SpeedButton18;
	TSplitView *svadauga;
	TImage *Image17;
	TImage *Image18;
	TSpeedButton *SpeedButton19;
	TSpeedButton *SpeedButton20;
	TImage *Image19;
	TSpeedButton *SpeedButton21;
	TGroupBox *GroupBox2;
	TLabel *Label23;
	TLabel *Label24;
	TLabel *Label25;
	TLabel *Label26;
	TLabel *Label27;
	TLabel *Label28;
	TLabel *Label29;
	TLabel *Label30;
	TLabel *Label31;
	TLabel *Label32;
	TLabel *Label33;
	TLabel *Label34;
	TImage *Image20;
	TImage *Image21;
	TSpeedButton *SpeedButton22;
	TSpeedButton *SpeedButton23;
	TImage *Image22;
	TSpeedButton *SpeedButton24;
	TImage *Image23;
	TSpeedButton *SpeedButton25;
	TEdit *Edit15;
	TEdit *Edit16;
	TEdit *Edit17;
	TEdit *Edit18;
	TEdit *Edit19;
	TEdit *Edit20;
	TEdit *Edit21;
	TEdit *Edit22;
	TEdit *Edit23;
	TEdit *Edit24;
	TEdit *Edit25;
	TEdit *Edit26;
	TDateTimePicker *DT;
	TSpeedButton *SpeedButton6;
	TPanel *Panel6;
	TLabel *Label35;
	TEdit *ecautare;
	TPanel *Panel7;
	TLabel *Label36;
	TEdit *ecautarepant;
	TLabel *Ltotal;
	TLabel *Ltotalafis;
	TLabel *Ltotalafisciz;
	TLabel *Label38;
	TMonthCalendar *DTshow;
	TMonthCalendar *DTshow2;
	TDateTimePicker *DT1;
	TCheckBox *CBintrariIarna;
	TCheckBox *CBelimIarna;
	TCheckBox *CBintrari;
	TCheckBox *CBelim;
	TSpeedButton *SpeedButton7;
	TRadioGroup *RadioGroup1;
	TRadioButton *RBPANTOFI;
	TRadioButton *RBCIZME;
	TPanel *Panel1;
	TPageControl *SPMG;
	TTabSheet *MGPantof;
	TDBGrid *DBGrid10;
	TGroupBox *GroupBox3;
	TLabel *Label7;
	TLabel *Label8;
	TLabel *Label10;
	TLabel *Label37;
	TLabel *Label39;
	TLabel *Label40;
	TLabel *Label41;
	TLabel *Label42;
	TLabel *Label43;
	TLabel *Label44;
	TLabel *Label45;
	TLabel *Label46;
	TImage *Image3;
	TImage *Image4;
	TSpeedButton *SpeedButton9;
	TSpeedButton *SpeedButton10;
	TImage *Image7;
	TSpeedButton *SpeedButton11;
	TImage *Image9;
	TSpeedButton *SpeedButton26;
	TEdit *Edit27;
	TEdit *Edit28;
	TEdit *Edit29;
	TEdit *Edit30;
	TEdit *Edit31;
	TEdit *Edit32;
	TEdit *Edit33;
	TEdit *Edit34;
	TEdit *Edit35;
	TEdit *Edit36;
	TEdit *Edit37;
	TDateTimePicker *DateTimePicker1;
	TTabSheet *MGCizme;
	TDBGrid *DBGrid9;
	TGroupBox *GroupBox4;
	TLabel *Label47;
	TLabel *Label48;
	TLabel *Label49;
	TLabel *Label50;
	TLabel *Label51;
	TLabel *Label52;
	TLabel *Label53;
	TLabel *Label54;
	TLabel *Label55;
	TLabel *Label56;
	TLabel *Label57;
	TLabel *Label58;
	TImage *Image24;
	TImage *Image25;
	TSpeedButton *SpeedButton27;
	TSpeedButton *SpeedButton28;
	TImage *Image26;
	TSpeedButton *SpeedButton29;
	TImage *Image27;
	TSpeedButton *SpeedButton30;
	TEdit *Edit39;
	TEdit *Edit40;
	TEdit *Edit41;
	TEdit *Edit42;
	TEdit *Edit43;
	TEdit *Edit44;
	TEdit *Edit45;
	TEdit *Edit46;
	TEdit *Edit47;
	TEdit *Edit48;
	TEdit *Edit49;
	TEdit *Edit50;
	TDateTimePicker *DateTimePicker2;
	TRadioGroup *RadioGroup2;
	TRadioButton *RBAPANTOF;
	TRadioButton *RBACIZME;
	TEdit *Edit38;
	TPanel *Panel5;
	TLabel *Label59;
	TLabel *Label60;
	TLabel *Label61;
	TEdit *Edit51;
	TCheckBox *CheckBox1;
	TCheckBox *CheckBox2;
	TMonthCalendar *MC;
	TPanel *Panel8;
	TLabel *Label62;
	TLabel *Label63;
	TLabel *Label64;
	TEdit *Edit52;
	TCheckBox *CheckBox3;
	TCheckBox *CheckBox4;
	TMonthCalendar *MC1;
	TRadioGroup *RadioGroup3;
	TRadioButton *RadioButton1;
	TRadioButton *RadioButton2;
	void __fastcall SplitView1MouseEnter(TObject *Sender);
	void __fastcall FormShow(TObject *Sender);
	void __fastcall SpeedButton1Click(TObject *Sender);
	void __fastcall SpeedButton2Click(TObject *Sender);
	void __fastcall SBafiseazaClick(TObject *Sender);
	void __fastcall SBmodificaClick(TObject *Sender);
	void __fastcall SBadaugaClick(TObject *Sender);
	void __fastcall SpeedButton12Click(TObject *Sender);
	void __fastcall SpeedButton13Click(TObject *Sender);
	void __fastcall SpeedButton15Click(TObject *Sender);
	void __fastcall DBGrid3DblClick(TObject *Sender);
	void __fastcall SpeedButton14Click(TObject *Sender);
	void __fastcall SpeedButton17Click(TObject *Sender);
	void __fastcall SpeedButton16Click(TObject *Sender);
	void __fastcall Edit3Click(TObject *Sender);
	void __fastcall DBGrid5DblClick(TObject *Sender);
	void __fastcall DBGrid6DblClick(TObject *Sender);
	void __fastcall Edit5Click(TObject *Sender);
	void __fastcall DBGrid7DblClick(TObject *Sender);
	void __fastcall Edit6Click(TObject *Sender);
	void __fastcall SpeedButton18Click(TObject *Sender);
	void __fastcall SpeedButton19Click(TObject *Sender);
	void __fastcall SpeedButton20Click(TObject *Sender);
	void __fastcall SpeedButton21Click(TObject *Sender);
	void __fastcall DBGrid1DblClick(TObject *Sender);
	void __fastcall Edit16Change(TObject *Sender);
	void __fastcall Edit26Click(TObject *Sender);
	void __fastcall Edit16Click(TObject *Sender);
	void __fastcall Edit17Click(TObject *Sender);
	void __fastcall SpeedButton25Click(TObject *Sender);
	void __fastcall DBGrid4DblClick(TObject *Sender);
	void __fastcall SpeedButton22Click(TObject *Sender);
	void __fastcall ecautareChange(TObject *Sender);
	void __fastcall ecautarepantChange(TObject *Sender);
	void __fastcall DTshow2Click(TObject *Sender);
	void __fastcall RBintrariClick(TObject *Sender);
	void __fastcall RBeliminariClick(TObject *Sender);
	void __fastcall SpeedButton24Click(TObject *Sender);
	void __fastcall Edit4Change(TObject *Sender);
	void __fastcall Edit15Change(TObject *Sender);
	void __fastcall SpeedButton23Click(TObject *Sender);
	void __fastcall CBintrariIarnaClick(TObject *Sender);
	void __fastcall CBelimIarnaClick(TObject *Sender);
	void __fastcall CBintrariClick(TObject *Sender);
	void __fastcall RBPANTOFIClick(TObject *Sender);
	void __fastcall RBCIZMEClick(TObject *Sender);
	void __fastcall SpeedButton5Click(TObject *Sender);
	void __fastcall SpeedButton3Click(TObject *Sender);
	void __fastcall SpeedButton4Click(TObject *Sender);
	void __fastcall RBAPANTOFClick(TObject *Sender);
	void __fastcall RBACIZMEClick(TObject *Sender);
	void __fastcall DBGrid10DblClick(TObject *Sender);
	void __fastcall DBGrid9DblClick(TObject *Sender);
	void __fastcall Edit38Click(TObject *Sender);
	void __fastcall Edit50Click(TObject *Sender);
	void __fastcall Edit39Click(TObject *Sender);
	void __fastcall Edit28Click(TObject *Sender);
	void __fastcall Edit40Click(TObject *Sender);
	void __fastcall Edit29Click(TObject *Sender);
	void __fastcall Edit41Click(TObject *Sender);
	void __fastcall SpeedButton26Click(TObject *Sender);
	void __fastcall SpeedButton9Click(TObject *Sender);
	void __fastcall Edit27Change(TObject *Sender);
	void __fastcall SpeedButton27Click(TObject *Sender);
	void __fastcall SpeedButton10Click(TObject *Sender);
	void __fastcall SpeedButton28Click(TObject *Sender);
	void __fastcall SpeedButton29Click(TObject *Sender);
	void __fastcall SpeedButton11Click(TObject *Sender);
	void __fastcall Edit52Change(TObject *Sender);
	void __fastcall Edit51Change(TObject *Sender);
	void __fastcall CheckBox1Click(TObject *Sender);
	void __fastcall MC1Click(TObject *Sender);
	void __fastcall CheckBox3Click(TObject *Sender);
	void __fastcall CheckBox4Click(TObject *Sender);
	void __fastcall MCClick(TObject *Sender);
	void __fastcall DTshowClick(TObject *Sender);
	void __fastcall CheckBox2Click(TObject *Sender);
	void __fastcall RadioButton2Click(TObject *Sender);
	void __fastcall RadioButton1Click(TObject *Sender);

private:	// User declarations
public:		// User declarations
	__fastcall TForm1(TComponent* Owner);
    int a;
};
//---------------------------------------------------------------------------
extern PACKAGE TForm1 *Form1;
//---------------------------------------------------------------------------
#endif
