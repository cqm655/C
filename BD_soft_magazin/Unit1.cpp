...
if(!dm->qliber->IsEmpty())
{
	...
	if((dm->qvanzare->Active==false ) || (dm->qvanzare->IsEmpty()))
	{
		//formam cecul
		 dm->FDStoredProc1->Close();
  dm->proccec->Params->Items[1]->AsInteger=Fautentificare->utilizator;  //opera2tor
   dm->proccec->Params->Items[2]->AsInteger=CLIENT; //client
  dm->proccec->Params->Items[3]->AsInteger=1;   //firma
  dm->proccec->ExecProc();
  CEC=dm->proccec->Params->Items[0]->AsInteger;
 ...
 ////////control FACEM INSERT SAU UPDATE, DACA EXISTA ASA PRODUS, PE BAZA DE COD PRODUS

 s="select v.* from VANZARE v inner join RECEPTIE r on r.RECEPTIE_ID=v.RECEPETIE_ID ";
 s+=" inner join PRODUSE p on p.PRODUSE_ID=r.PRODUSE_ID ";
 s+="  where p.COD_PRODUS=:COD_PRODUS and v.CEC_ID=:CEC_ID ";
dm->qliber2->Close();
dm->qliber2->SQL->Clear();
dm->qliber2->SQL->Add(s);
dm->qliber2->ParamByName("cec_id")->AsInteger=CEC;
dm->qliber2->ParamByName("cod_produs")->AsString=Edit1->Text;
dm->qliber2->Open();
...
else
{
 ...
 s=" update VANZARE set cantitatea=cantitatea+1, suma=suma+ :suma, suma_disc=suma_disc+ :suma_disc, suma_total=suma_total+ :suma_total ";
